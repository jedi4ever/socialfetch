...
func TestXProviderFetchBuildsFromQuery(t *testing.T) {
	...
	p := NewXProvider(fake)
	item, err := p.Fetch(context.Background(), "swyx", Options{Kind: "tweets", Max: 5})
	if err != nil {
...
func TestXProviderUnknownKind(t *testing.T) {
	...
	_, err := p.Fetch(context.Background(), "swyx", Options{Kind: "garbage"})
...
func TestXProviderRejectsEmptyUser(t *testing.T) {
	p := NewXProvider(&fakeXSearcher{})
	_, err := p.Fetch(context.Background(), "", Options{})
	if err == nil {
