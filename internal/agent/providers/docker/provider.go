...
// rewriteLoopbackURL is a thin alias kept for in-package callers;
// the real implementation lives at agent.RewriteLoopbackURL so
// other host-side callers (cmd/social-researcher, etc) can share it.
func rewriteLoopbackURL(v string) string {
	return agent.RewriteLoopbackURL(v)
}
